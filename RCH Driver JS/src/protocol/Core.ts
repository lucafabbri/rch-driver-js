...
export class Core {
     ...
     * @param {Number} qty
     */
    departmentSell(departmentId: number, description: string, value: number, qty = 1): string {
        var command = "=R" + departmentId + "/$" + Math.floor(value) + "/(" + description + ")" + "/*" + Math.floor(qty)
    ...
    departmentSellFromLineItem(lineItem: LineItemDTO): string {
        return this.departmentSell(lineItem.departmentId, lineItem.description, lineItem.price, lineItem.quantity);
    }
    /**
    ...
    paymentFromPaymentItem(paymentItem: PaymentItemDTO): string {
        return this.payment(paymentItem.paymentId, paymentItem.value, paymentItem.description);
    }
    /**
     * 
     * @param {String} date 
